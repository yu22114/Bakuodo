...
// EditCypherScreenとほぼ同じ作り。限定公開・参加承認制の項目がない分シンプル
export function EditNumberScreen({ numberId, user, onBack, onSaved }: {
  ...
  useEffect(() => {
    async function fetchNumber() {
      const { data } = await supabase.from("numbers")
        .select("id, title, starts_at, ends_at, location, description, max_members, studio_fee, number_genres(genres:genre_id(name))")
  ...
  const handleSave = async () => {
    ...
    const { error: uErr } = await supabase.from("numbers").update({ title, location, description: form.description, starts_at, ends_at, max_members: form.max_members ? Number(form.max_members) : null, studio_fee: form.studio_fee ? Number(form.studio_fee) : null }).eq("id", numberId).eq("organizer_id", user.id);
  ...
  return (
    <div {...swipeBack} style={{ paddingBottom: "80px", background: "#000000" }}>
      ...
      <div style={{ padding: "20px 16px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <div><label style={lbl}>最寄り駅 <span style={{ color: "#EC4899" }}>*</span></label><StationSearch value={form.station} onChange={v => setForm(f => ({ ...f, station: v }))} inputStyle={inp} /></div>
        ...
        <div><label style={lbl}>作品名 <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "8px" }}>任意</span></label><input style={inp} placeholder="空欄の場合は開催場所がタイトルになります" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} /></div>
        <div>
        ...
        <button onClick={handleSave} disabled={saving}
          style={{ width: "100%", padding: "14px", border: "none", borderRadius: "6px", background: form.date && form.station ? "#EC4899" : "rgba(255,255,255,0.08)", color: form.date && form.station ? "#fff" : "rgba(255,255,255,0.3)", fontSize: "15px", fontFamily: "'Noto Sans JP',sans-serif", fontWeight: 700, letterSpacing: "0.15em", cursor: form.date && form.station ? "pointer" : "not-allowed", opacity: saving ? 0.6 : 1 }}>
