        ...
        <div style={{ overflowY: "auto", padding: "8px 20px 0", flex: 1 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "16px" }}>
            ...
            <button onClick={() => onViewProfile(lesson.organizer.id)} style={{ display: "flex", gap: "10px", fontSize: "13px", color: "#111111", fontFamily: "'Noto Sans JP',sans-serif", alignItems: "center", background: "none", border: "none", cursor: "pointer", padding: 0, textAlign: "left", textDecoration: "underline dotted", textUnderlineOffset: "3px" }}>
              <User size={14} color="rgba(0,0,0,0.4)" /> {isEvent ? "主催" : "講師"}: {lesson.organizer.dancer_name}
            </button>
            {lesson.price != null && (
              <div style={{ display: "flex", gap: "10px", fontSize: "13px", color: "#111111", fontFamily: "'Noto Sans JP',sans-serif", alignItems: "center" }}>
          ...
          {!isOwn && (
            isEnded ? (
              ...
              <div style={{ marginTop: "20px", padding: "14px", background: accent + "10", border: "1px solid " + accent + "33", borderRadius: "6px", textAlign: "center", fontSize: "13px", color: accent, fontFamily: "'Noto Sans JP',sans-serif" }}>
                定員に達しています（{participants.length}/{lesson.max_members}人）
              </div>
            ) : (
              <button onClick={() => { onJoin(lesson.id); if (!joined && !pending) onClose(); }}
                style={{ marginTop: "20px", width: "100%", padding: "14px", border: "none", borderRadius: "6px", background: joined ? "rgba(22,163,74,0.1)" : pending ? "rgba(0,0,0,0.06)" : accent, color: joined ? "#16A34A" : pending ? "rgba(0,0,0,0.45)" : "#fff", fontSize: "14px", fontFamily: "'Noto Sans JP',sans-serif", fontWeight: 700, letterSpacing: "0.15em", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
